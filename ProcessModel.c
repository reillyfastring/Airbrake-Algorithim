...
void processModel(double sigma, int n, double t, double* DeployAngle, double* sensorVector) {
    ...
    for (int i = 1; i < (n+1); i++) {
        ...
        sigma[i*SIZE + 1] += drag(DeployAngle, sigma[i*SIZE + 2]);
